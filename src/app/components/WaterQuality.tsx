...
export function WaterQuality() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      setReadings(JSON.parse(storedData).reverse()); // Most recent first
    } else {
      // Generate some mock data if none exists
      const mockData: WaterQualityReading[] = Array.from({ length: 15 }, (_, i) => {
        const date = new Date();
        ...
        return {
          ...
          turbidity: (0.5 + Math.random() * 1.0).toFixed(1),
  ...
  const getStatus = (reading: WaterQualityReading) => {
    const ph = parseFloat(reading.ph);
    const turbidity = parseFloat(reading.turbidity);
    const chlorine = parseFloat(reading.chlorine);

    if (ph < 6.5 || ph > 8.5 || turbidity > 1.5 || chlorine < 0.5 || chlorine > 3.0) {
      return 'alert';
    }
    if (ph < 7.0 || ph > 8.0 || turbidity > 1.0 || chlorine < 1.0 || chlorine > 2.5) {
      return 'warning';
    }
    return 'normal';
  };
  ...
  const filteredReadings = filter === 'all' 
    ? readings 
    : readings.filter(r => getStatus(r) === filter);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1>Water Quality Readings</h1>
          <p className="text-gray-600 mt-1">
            View and analyze all water quality measurements
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-semibold mt-1">{statusCounts.normal}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-semibold mt-1">{statusCounts.warning}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-semibold mt-1">{statusCounts.alert}</p>
              </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>All water quality measurements and their status</CardDescription>
            </div>
            <div className="w-40">
                  ...
                  filteredReadings.map((reading) => {
                    ...
                    return (
                      <TableRow key={reading.id}>
                        ...
                        <TableCell>{reading.turbidity} NTU</TableCell>
                        <TableCell>{reading.chlorine} mg/L</TableCell>
                        <TableCell>{reading.flowRate} L/min</TableCell>
                        <TableCell>{reading.temperature}°C</TableCell>
                        <TableCell className="text-sm">
                          {formatLocation(reading.location)}
                        </TableCell>
                        ...
                        <TableCell>
                          ...
                          >
                            {status === 'alert' && <AlertCircle className="w-3 h-3 mr-1" />}
